...
public class ClusterManager {
    ...
    public ClusterNode getShardedNode(String[] command)
        ...
        // Commands with no key → just route to the first node (e.g., management commands)
        if (command.length < 3) {
    ...
    /**
     * Delegate a read to an existing or newly chosen healthy node. For GETs, it's safe to route to
