package com.danieljhkim.kvdb.kvclustercoordinator.config;
...
/**
 * Configuration class for cluster nodes. Loads and manages cluster node configurations from a YAML
 * file.
 */
public class ClusterConfig {
        ...
        try (InputStream input = getClass().getClassLoader().getResourceAsStream(configFilePath)) {
            if (input == null) {
                throw new IllegalArgumentException(
                        "Cluster config file not found in classpath: " + configFilePath);
            }
            loadConfig(input);
        } catch (IOException e) {
    ...
    public ClusterConfig(File configFile) {
        try (InputStream input = new FileInputStream(configFile)) {
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to load cluster config from file: " + configFile.getPath(),
                    e);
            throw new RuntimeException("Failed to load cluster config", e);
        }
    }
    ...
    private void loadConfig(InputStream input) {
        try {
            ...
            for (Map<String, Object> item : nodesList) {
                ...
                boolean useGrpc =
                        item.getOrDefault("useGrpc", true) instanceof Boolean
    ...
    private void validateNodeConfig(Map<String, Object> item) {
        ...
        if (!item.containsKey("host") || !(item.get("host") instanceof String)) {
            throw new IllegalArgumentException(
                    "Node configuration missing or invalid 'host' field");
        }
        if (!item.containsKey("port") || !(item.get("port") instanceof Integer)) {
            throw new IllegalArgumentException(
                    "Node configuration missing or invalid 'port' field");
        }
